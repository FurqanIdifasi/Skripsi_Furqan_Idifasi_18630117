            ...
            <?php
             ...
             /* if($_SESSION['hak_akses'] == 'pelanggan') { 
                 $id = $_SESSION['user_id'];
                  ...
                  $datas = mysqli_query($koneksi, "select pendaftaran.*, pelanggan.nama,pelanggan.alamat, paket.nama as nama_paket from pendaftaran JOIN pelanggan ON pelanggan.id = pendaftaran.pelanggan_id join paket on paket.id = pendaftaran.paket_id   WHERE YEAR(pendaftaran.tgl_psb) = '$tah' AND MONTH(pendaftaran.tgl_psb) = '$bul' group by pendaftaran.id") or die(mysqli_error($koneksi)); 
              /*}
*/
