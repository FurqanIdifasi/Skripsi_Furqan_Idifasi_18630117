...
$data_cek   = mysqli_query($koneksi, "select * from pelanggan where nik = '$nik'");
          ...
          $datas = mysqli_query($koneksi, "insert into pelanggan (nama,hp,alamat,status_rumah,longitude,latitude,kode,foto_pelanggan,foto_ktp,foto_selfie,foto_rumah,nik,tgl_daftar)values('$nama','$hp','$alamat','$status_rumah','$longitude','$latitude','$kode','$foto_pelanggan','$foto_ktp','$foto_selfie','$foto_rumah','$nik','$tgl_daftar')") or die(mysqli_error($koneksi));

          $pelanggan_id = mysqli_insert_id($koneksi);
        }

          $datas2 = mysqli_query($koneksi, "insert into pendaftaran (pelanggan_id,no_psb,paket_id,tgl_psb,status_pemasangan,jenis_psb)values('$pelanggan_id','$no_psb','$paket_id','$tgl_psb','$status_pemasangan','$jenis_psb')") or die(mysqli_error($koneksi));


          $datas22 = mysqli_query($koneksi, "insert into karyawan (pem_id,nama,nik,no_hp,alamat,jabatan,tim,area,foto,username,password)values('$pelanggan_id','$nama','$nik','$hp','$alamat','$jabatan','$tim','$area','$foto_pelanggan','$nik','$nik')") or die(mysqli_error($koneksi));
